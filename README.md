...
`local_model_size` in `%APPDATA%\WisprLite\config.json` to `small.en`,
...
Settings persist to `%APPDATA%\WisprLite\config.json`. API keys live only in
...
1. **Single .exe** — run **`build_exe.bat`** → `dist\WisprLite.exe` (PyInstaller,
...
2. **Full installer** — install [Inno Setup 6](https://jrsoftware.org/isdl.php),
   ...
   `installer\Output\WisprLite-Setup.exe`: a per-user install (no admin /UAC),
